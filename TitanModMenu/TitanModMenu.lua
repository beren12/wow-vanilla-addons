...
function TitanPanelModMenu_GetOnDemandText(v)
	local color;
	local stat = TitanPanelModMenu_GetStatus(v);
	
	if (stat == 0) then 			-- Grey (Not Loaded, Disabled)
		color = "ff9d9d9d";	
	elseif (stat == 1) then 	-- Orange (Not Loaded, Enabled, Dependancy not loaded)
		color = "ffff8000";	
	elseif (stat == 2) then 	-- Green (Not Loaded, Enabled) aka OnDemand
		color = "ff1eff00"; 
	elseif (stat == 3) then		-- White (Loaded, Enabled) 
		return v.title;
	else 											-- Purple (Loaded, Disabled)
		color = "ffa335ee"; 
	end	

	return "|c".. color.. v.title.. FONT_COLOR_CODE_CLOSE;
end

function TitanPanelModMenu_GetStatus(v)	
	if ((not v.loadable) and (not v.loaded) and (not v.enabled)) then 
		-- (Not Loaded, Disabled)
		return 0;
	elseif ((not v.enabled) and (not v.loadable)) then 
		-- (Loaded, Disabled)
		return 4;
	elseif (not v.loaded) then 
		if (v.reason) then 
			-- (Not Loaded, Enabled, Dependancy not loaded)
			return 1;
		else
			-- (Not Loaded, Enabled) aka OnDemand
			return 2;
		end
	end
	-- (Loaded, Enabled)
