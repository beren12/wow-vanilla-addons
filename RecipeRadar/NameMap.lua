
-- NameMap.lua: recipes named differently after learning
-- Auto generated on 9/17/2006 3:42:09 PM

RecipeRadar_NameMap = {

   ["enUS"] = {
      -- Alchemy
      ["Philosophers' Stone"] = "Philosopher's Stone",
      ["Transmute: Iron to Gold"] = "Transmute Iron to Gold",
      ["Transmute: Mithril to Truesilver"] = "Transmute Mithril to Truesilver",
      ["Transmute: Arcanite"] = "Transmute Arcanite",
      ["Transmute: Air to Fire"] = "Transmute Air to Fire",
      ["Transmute: Earth to Water"] = "Transmute Earth to Water",
      ["Transmute: Fire to Earth"] = "Transmute Fire to Earth",
      ["Transmute: Water to Air"] = "Transmute Water to Air",
      ["Greater Dreamless Sleep Potion"] = "Greater Dreamless Sleep",
      ["Transmute: Elemental Fire"] = "Transmute Elemental Fire",
      -- Enchanting
      ["Enchant Bracer - Healing Power"] = "Enchant Bracer - Healing",
      -- Cooking
      ["Mithril Headed Trout"] = "Mithril Head Trout",
   },

   ["enGB"] = {
      -- Alchemy
      ["Philosophers' Stone"] = "Philosopher's Stone",
      ["Transmute: Iron to Gold"] = "Transmute Iron to Gold",
      ["Transmute: Mithril to Truesilver"] = "Transmute Mithril to Truesilver",
      ["Transmute: Arcanite"] = "Transmute Arcanite",
      ["Transmute: Air to Fire"] = "Transmute Air to Fire",
      ["Transmute: Earth to Water"] = "Transmute Earth to Water",
      ["Transmute: Fire to Earth"] = "Transmute Fire to Earth",
      ["Transmute: Water to Air"] = "Transmute Water to Air",
      ["Greater Dreamless Sleep Potion"] = "Greater Dreamless Sleep",
      ["Transmute: Elemental Fire"] = "Transmute Elemental Fire",
      -- Enchanting
      ["Enchant Bracer - Healing Power"] = "Enchant Bracer - Healing",
      -- Cooking
      ["Mithril Headed Trout"] = "Mithril Head Trout",
   },

   ["deDE"] = {
      -- Alchemy
      ["Transmutieren: Eisen in Gold"] = "Eisen in Gold transmutieren",
      ["Transmutieren: Mithril in Echtsilber"] = "Mithril in Echtsilber transmutieren",
      ["Transmutieren: Arkanit"] = "Arkanit transmutieren",
      ["Transmutieren: Luft zu Feuer"] = "Luft zu Feuer transmutieren",
      ["Transmutieren: Erde zu Wasser"] = "Erde zu Wasser transmutieren",
      ["Transmutieren: Feuer zu Erde"] = "Feuer zu Erde transmutieren",
      ["Transmutieren: Wasser zu Luft"] = "Wasser zu Luft transmutieren",
      -- Blacksmithing
      ["Dunkeleisenhandschuhe"] = "Dunkeleisenstulpen",
      -- Enchanting
      ["Waffe verzaubern - Stärke"] = "Waffe - Stärke",
      ["Waffe verzaubern - Beweglichkeit"] = "Waffe - Beweglichkeit",
      ["Armschiene verzaubern - Manaregeneration"] = "Armschiene - Manaregeneration",
      ["Armschiene verzaubern - Heilkraft"] = "Armschiene - Heilung",
      ["Waffe verzaubern - mächtige Willenskraft"] = "Waffe - Mächtige Willenskraft",
      ["Waffe verzaubern - mächtige Intelligenz"] = "Waffe - Mächtige Intelligenz",
      ["Formel: Umhang verzaubern - Großer Feuerwiderstand"] = "Umhang - Großer Feuerwiderstand",
      ["Formel: Umhang verzaubern - Großer Naturwiderstand"] = "Umhang - Großer Naturwiderstand",
      -- Engineering
      ["Mechanischer Großdrachling"] = "Mechanischer Drachling",
      ["EZ-Thro Dynamit II"] = "EZ-Thro-Dynamit II",
      ["Tieftauchhelm"] = "Tiefentaucherhelm",
      ["Mechanischer Mithrilgroßdrachling"] = "Mechanischer Mithrildrachling",
      ["Echtsilber-Umwandler"] = "Echtsilberumwandler",
      ["Gyrofrosteisdeflektor"] = "Gyrofrosteisreflektor",
      -- Leatherworking
      ["Murlocschuppen-Gürtel"] = "Murlocschuppengürtel",
      ["Murlocschuppen-Brustplatte"] = "Murlocschuppenbrustplatte",
      ["Murlocschuppen-Armschienen"] = "Murlocschuppenarmschienen",
      ["Grünwelpenarmschienen"] = "Grünwelpen-Armschienen",
      ["Urzeitliche Fledermaushautwams"] = "Urzeitliches Fledermaushautwams",
      -- Cooking
      ["Redridge-Gulasch"] = "Rotkammgulasch",
   },

   ["frFR"] = {
      -- Alchemy
      ["Potion de Rage"] = "Potion de rage",
      ["Potion de protection contre le sacré"] = "Potion de protection contre le Sacré",
      ["Potion de protection contre le feu"] = "Potion de protection contre le Feu",
      ["Potion de protection contre le givre"] = "Potion de protection contre le Givre",
      ["Potion de protection contre la nature"] = "Potion de protection contre la Nature",
      ["Transmutation : Fer en Or"] = "Transmutation du fer en or",
      ["Transmutation : Mithril en vrai-argent"] = "Transmutation du mithril en vrai-argent",
      ["Elixir de défense excellent"] = "Elixir de Défense excellente",
      ["Potion de soins majeure"] = "Potion de Soins majeure",
      ["Potion de sommeil sans rêve supérieure"] = "Sommeil sans rêve supérieur",
      ["Transmutation : feu élémentaire"] = "Transmutation du feu élémentaire",
      -- Enchanting
      ["Ench. de plastron (Mana mineur)"] = "Enchantement de plastron (Mana mineur)",
      ["Ench. de plastron (Mana inférieur)"] = "Enchantement de plastron (Mana inférieur)",
      ["Ench. d'arme 2M (Intelligence inférieure)"] = "Enchantement d'arme 2M (Intelligence inférieure)",
      ["Ench. de cape (Agilité mineure)"] = "Enchantement de cape (Agilité mineure)",
      ["Ench. de bottes (Agilité mineure)"] = "Bottes enchantées (Agilité mineure)",
      ["Ench. de bracelets (Force inférieure)"] = "Enchantement de bracelets (Force inférieure)",
      ["Ench. de bracelets (Déviation inférieure)"] = "Enchantement de bracelets (Déviation inférieure)",
      ["Ench. de bracelets (Déviation)"] = "Enchantement de bracelets (Déviation)",
      ["Ench. de bouclier (Endurance supérieure)"] = "Enchantement de bouclier (Endurance supérieure)",
      ["Ench. de plastron (Vie majeure)"] = "Enchantement de plastron (Santé majeure)",
      ["Ench. de cape (Défense excellente)"] = "Enchantement de cape (Défense excellent)",
      ["Ench. d'arme (Force)"] = "Enchantement d'arme (Force)",
      ["Ench. d'arme (Agilité)"] = "Enchantement d'arme (Agilité)",
      ["Ench. de bracelets (Régénération de mana)"] = "Enchantement de bracelets (Régénération de mana)",
      ["Ench. de bracelets (Pouvoir de guérison)"] = "Enchantement de bracelets (Soin)",
      ["Ench. d'arme (Esprit renforcé)"] = "Enchantement d'arme (Esprit renforcé)",
      ["Ench. d'arme (Intelligence renforcée)"] = "Enchantement d'arme (Intelligence renforcée)",
      ["Ench. de cape (Résistance au Feu supérieure)"] = "Enchantement de cape (Résistance au feu supérieure)",
      ["Ench. de cape (Résistance à la Nature supérieure)"] = "Enchantement de cape (Résistance à la nature supérieure)",
      -- Engineering
      ["Dynamite Ez-Thro II"] = "Dynamite Ev-Lan II",
      ["Feu d'artifice \"Explosion de serpent\""] = "Feu d'artifice \"Explosion serpentine\"",
      ["Rouage en thorium"] = "Rouages en thorium",
      ["Puissante charge de seaforium"] = "Puissante charge d'hydroglycérine",
      -- Leatherworking
      ["Plastron en écailles de murloc"] = "Cuirasse en écailles de murloc",
      ["Cape en cuir de dragonnet noir"] = "Cape de dragonnet noir",
      ["Tunique en cuir de dragonnet noir"] = "Tunique de dragonnet noir",
      ["Armure de murloc épaisse"] = "Armure épaisse de murloc",
      ["Ceinture en cuir clouté de gemmes"] = "Ceinture en cuir cloutée de gemmes",
      ["Bracelets en écailles de murloc"] = "Brassards en écailles de murloc",
      ["Bracelets en cuir de dragonnet vert"] = "Brassards de dragonnet vert",
      ["Bracelets en écailles de scorpide épaisses"] = "Brassards en écailles de scorpide épaisses",
      ["Gants grumegueules"] = "Batailleurs grumegueules",
      ["Plastron du tigre-sang"] = "Cuirasse du tigre-sang",
      ["Tunique en peau de chauve-souris primordiale"] = "Pourpoint en peau de chauve-souris primordiale",
      ["Plastron de crache-feu"] = "Cuirasse de crache-feu",
      ["Plastron de traqueuse des sables"] = "Cuirasse de traqueuse des sables",
      ["Plastron de rêvécaille"] = "Cuirasse de rêvécaille",
      ["Plastron en écailles de dragon vert"] = "Cuirasse en écailles de dragon vert",
      ["Plastron en écailles de dragon bleu"] = "Cuirasse en écailles de dragon bleu",
      ["Plastron en écailles de dragon noir"] = "Cuirasse en écailles de dragon noir",
      ["Harnais des grands ours"] = "Harnais de l'ours de guerre",
      ["Jambières des grands ours"] = "Jambières de l'ours de guerre",
      -- Tailoring
      ["Veste bleue en lin"] = "Gilet bleu en lin",
      ["Combinaison bleue"] = "Salopette bleue",
      ["Robe d'adepte supérieure"] = "Robe d'adepte supérieur",
      ["Chemise lavande en tissu de mage"] = "Chemise lavande en tisse-mage",
      ["Chemise rose en tissu de mage"] = "Chemise rose en tisse-mage",
      ["Jambières en vignesang"] = "Jambières  en vignesang",
      ["Veste en vignesang"] = "Gilet en vignesang",
      -- Cooking
      ["Biscuit au pain d'épices"] = "Biscuit de pain d'épice",
      ["Chilli de souffle de dragon"] = "Chili de souffle de dragon",
      ["Chilli de crabe épicé"] = "Chili de crabe épicé",
      -- First Aid
      ["Bandage en tissu de mage"] = "Bandage en tisse-mage",
   },

}

