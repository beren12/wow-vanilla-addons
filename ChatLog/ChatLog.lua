...
function ChatLog_OnLoad()
	...
	ChatLogMaxLogInfoText:SetTextColor(1.0, 1.0, 1.0);
...
function ChatLog_OnEvent(event)
		...
		if ( arg1 == "ChatLog" ) then
			...
			for index=1, getn(ChatLog_Logs) do
				if (ChatLog_Logs[index]["enabled"] == 1) then
					...
					if (index == CHAT_LOG_CURRENT_INDEX) then
	...
	if ( event == "CHAT_MSG_CHANNEL_NOTICE" ) then
		if (arg1 == "YOU_JOINED") then
			if (not (arg8 == nil)) then
				if (not ChatLog_IsLogStructureDefined(arg8)) then
					ChatLog_CreateNewLogStructure(getn(ChatLog_Logs)+1, arg8);
					if (CHAT_LOG_ENABLED_DEFAULT == 1) then
						ChatLog_AddHeader(index);
					end
				end
		...
		elseif (arg1 == "YOU_LEFT") then
			if (not (arg8 == nil)) then
				if (ChatLog_MakeID(arg8) == ChatLog_Logs[CHAT_LOG_CURRENT_INDEX]["id"]) then
					ChatLogDelete:Enable();
					ChatLog_ShowLog(CHAT_LOG_CURRENT_INDEX);
				end
			end
		end
	...
	if ( event == "CHAT_MSG_WHISPER" and ChatLog_Logs[CHAT_LOG_WHISPER_INDEX]["enabled"] == 1) then
		...
		tinsert (ChatLog_Logs[CHAT_LOG_WHISPER_INDEX]["logs"], msg);
	...
	if ( ((event == "CHAT_MSG_RAID") or (event == "CHAT_MSG_RAID_LEADER") or (event == "CHAT_MSG_RAID_WARNING")) and ChatLog_Logs[CHAT_LOG_RAID_INDEX]["enabled"] == 1) then
		...
		tinsert (ChatLog_Logs[CHAT_LOG_RAID_INDEX]["logs"], msg);
	...
	if ( event == "CHAT_MSG_PARTY" and ChatLog_Logs[CHAT_LOG_PARTY_INDEX]["enabled"] == 1) then
		...
		tinsert (ChatLog_Logs[CHAT_LOG_PARTY_INDEX]["logs"], msg);
	...
	if ( event == "CHAT_MSG_SAY" and ChatLog_Logs[CHAT_LOG_SAY_INDEX]["enabled"] == 1) then
		...
		tinsert (ChatLog_Logs[CHAT_LOG_SAY_INDEX]["logs"], msg);
	...
	if ( event == "CHAT_MSG_YELL" and ChatLog_Logs[CHAT_LOG_YELL_INDEX]["enabled"] == 1) then
		...
		tinsert (ChatLog_Logs[CHAT_LOG_YELL_INDEX]["logs"], msg);
	...
	if ( event == "CHAT_MSG_OFFICER" and ChatLog_Logs[CHAT_LOG_OFFICER_INDEX]["enabled"] == 1) then
		...
		tinsert (ChatLog_Logs[CHAT_LOG_OFFICER_INDEX]["logs"], msg);
	...
	if ( event == "CHAT_MSG_GUILD" and ChatLog_Logs[CHAT_LOG_GUILD_INDEX]["enabled"] == 1) then
		...
		tinsert (ChatLog_Logs[CHAT_LOG_GUILD_INDEX]["logs"], msg);
	...
	if (event == "CHAT_MSG_CHANNEL") then
		if (not (arg8 == nil)) then
			...
			index = ChatLog_GetCustomChatIndexByID(ChatLog_MakeID(name));
			
			if (not (index == 0)) then
				if (ChatLog_Logs[index]["enabled"] == 1) then
					msg = "[" .. date("%H:%M:%S") .. "] |Hplayer:" .. arg2 .. "|h[" .. arg2 .. "]|h: " .. arg1;
					tinsert (ChatLog_Logs[index]["logs"], msg);
					ChatLog_CheckTableSize(ChatLog_Logs[index]["logs"]);
	...
	if ( event == "PARTY_MEMBERS_CHANGED") then
		if ( not (GetNumPartyMembers() == CHAT_LOG_NUM_PARTY_MEMBERS) ) then
			...
			if (CHAT_LOG_NUM_PARTY_MEMBERS == 0 and GetNumPartyMembers() > 0) then
				tinsert (ChatLog_Logs[CHAT_LOG_PARTY_INDEX]["logs"], " ");
	...
	if ( event == "RAID_ROSTER_UPDATE" ) then
		if ( not (GetNumRaidMembers() == CHAT_LOG_NUM_RAID_MEMBERS) ) then
			...
			if (CHAT_LOG_NUM_RAID_MEMBERS == 0 and GetNumRaidMembers() > 0) then
				tinsert (ChatLog_Logs[CHAT_LOG_RAID_INDEX]["logs"], " ");
...
function ChatLog_GetCustomChatIndexByID(ChannelID)
	for index=8, getn(ChatLog_Logs) do
		if (ChannelID == ChatLog_Logs[index]["id"]) then
			return index;
		end
	end
...
function ChatLog_InitStructure()
	...
	--Resets the table at each loading
	--Chat IDs
	ChatLog_Logs[1]["id"] = CHAT_LOG_WHISPER_ID;
	ChatLog_Logs[2]["id"] = CHAT_LOG_RAID_ID;
	ChatLog_Logs[3]["id"] = CHAT_LOG_PARTY_ID;
	ChatLog_Logs[4]["id"] = CHAT_LOG_SAY_ID;
	ChatLog_Logs[5]["id"] = CHAT_LOG_YELL_ID;
	ChatLog_Logs[6]["id"] = CHAT_LOG_OFFICER_ID;
	ChatLog_Logs[7]["id"] = CHAT_LOG_GUILD_ID;
	--Chat index
	...
	--Default chat names
	ChatLog_Logs[1]["name"] = CHAT_LOG_WHISPER_NAME;
	ChatLog_Logs[2]["name"] = CHAT_LOG_RAID_NAME;
	ChatLog_Logs[3]["name"] = CHAT_LOG_PARTY_NAME;
	ChatLog_Logs[4]["name"] = CHAT_LOG_SAY_NAME;
	ChatLog_Logs[5]["name"] = CHAT_LOG_YELL_NAME;
	ChatLog_Logs[6]["name"] = CHAT_LOG_OFFICER_NAME;
	ChatLog_Logs[7]["name"] = CHAT_LOG_GUILD_NAME;
	...
	for index, value in ipairs(ChannelList) do
		if (type(value) == "number") then
			if (not ChatLog_IsLogStructureDefined(value)) then
				ChatLog_CreateNewLogStructure(getn(ChatLog_Logs)+1, value);
				if (CHAT_LOG_ENABLED_DEFAULT == 1) then
					ChatLog_AddHeader(index);
				end
			end
...
function ChatLog_IsPlayerOnChannel(ChannelID)
	local id, name;
	local cid, cname;
...
function ChatLog_CreateNewLogStructure(index, ChannelID)
	local id, name = GetChannelName(ChannelID);
	
	id = ChatLog_MakeID(name);
	
	ChatLog_Logs[index] = {};
	ChatLog_Logs[index]["id"] = id;
	ChatLog_Logs[index]["name"] = name;
	ChatLog_Logs[index]["enabled"] = CHAT_LOG_ENABLED_DEFAULT;
	ChatLog_Logs[index]["logs"] = {};
	
	--Depreciated, but won't work without this
	table.setn(ChatLog_Logs, index);
end
...
function ChatLog_AddHeader(index)
	if (not (getn(ChatLog_Logs[index]["logs"]) == 0)) then
		tinsert (ChatLog_Logs[index]["logs"], " ");
		tinsert (ChatLog_Logs[index]["logs"], " ");
	end
	
	tinsert (ChatLog_Logs[index]["logs"], "|c" .. ChatLog_MakeHex(CHAT_LOG_COLORS["SYSTEM"]["r"], CHAT_LOG_COLORS["SYSTEM"]["g"], CHAT_LOG_COLORS["SYSTEM"]["b"]) .. CHAT_LOG_LOGGING_STARTED_ON .. " " .. date("%m/%d/%Y") .. " " .. CHAT_LOG_LOGGING_STARTED_AT .. " " .. date("%H:%M:%S") .. ":|r");
end
...
function ChatLog_CheckTableSize(msgTable)
	local i, tableSize = 0, 0;
	
	tableSize = getn(msgTable);
	
	if ( tableSize >=  CHAT_LOG_MAXSIZE) then
		for i=1, tableSize-CHAT_LOG_MAXSIZE do
			tremove(msgTable, 1);
		end
...
function ChatLog_ToggleEnableThisLog()
	...
	if (ChatLog_Logs[CHAT_LOG_CURRENT_INDEX]["enabled"] == 1) then
		...
		tinsert (ChatLog_Logs[CHAT_LOG_CURRENT_INDEX]["logs"], msg);
...
function ChatLog_ExtractAndDisplayInfo (msgTable, currentMaxLine)
	...
	local line;
	for line = CHAT_LOG_CURRENT_MAXLINE-CHAT_LOG_SCROLLING_MESSAGE_FRAME_MAXLINES, CHAT_LOG_CURRENT_MAXLINE do
		if (line > 0) then
			ChatLog_AddLine(msgTable[line]);
		end
	end
...
function ChatLog_ExtractCurrentAndDisplayCopyInfo (msgTable)
	...
	local imin, imax;
	imin = getn(msgTable)-CHAT_LOG_COPY_MAXSIZE;
	if (imin < 1) then
		imin = 1;
	end
	imax = getn(msgTable);
	for i=imin, imax do
		ChatLogCopyEditBox:SetText(ChatLogCopyEditBox:GetText() .. msgTable[i] .. "\n");
	end
end
...
function ChatLog_SlashCommand(args)
	...
	if (arg1 == CHAT_LOG_PCLEAR) then
		index = ChatLog_GetCustomChatIndexByID(ChatLog_MakeID(arg2));
