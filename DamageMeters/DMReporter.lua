...
function DMReporter_OutputToCSV(outputFilename)
	...
	local index;
	for index = 1, #(DamageMeters_tables[DMT_ACTIVE]) do
...
function DMReporter_OutputEventsToCSV(outputFilename)
	...
	groupTotals = {};
	local ii;
	for ii = 1, #(DamageMeters_tables[DMT_ACTIVE]) do
		local q;
		for q = 1, DMI_MAX do
	...
	for index, struct in pairs(DamageMeters_tables[DMT_ACTIVE]) do
		...
		for dmi = 1, DMI_MAX do
			...
			if (quantityStruct) then
				...
				for spell, spellStruct in pairs(quantityStruct.spellTable) do
...
function DMReporter_OutputEventsTotalsCSV(outputFilename)
	...
	for quantity = 1, DMI_MAX do
		...
		for spell, spellStruct in pairs(quantityStruct) do
			local average = DM_GetFraction(spellStruct.value, spellStruct.counts[1]);
...
function DMReporter_OuptutClassReportCSV(outputFilename)
	...
	for ix = 1, #(DamageMeters_tables[DMT_ACTIVE]) do
	...
	for class, classInfo in pairs(classTable) do
...
------------------------------------------------------------------------
	...
	for plugin, dmi in pairs(DamageMeters_pluginDMITable) do
		if (dmi > DMI_MAX) then
	...
	if (nil == DamageMeters_tables[DMT_ACTIVE] or 0 == #(DamageMeters_tables[DMT_ACTIVE])) then
	...
	if (doTotalReport or doLeaderReport or doEventReport) then
		...
		if (doTotalReport) then
			DamageMeters_DoReport(DamageMeters_ReportQuantity_Total, "BUFFER", false, 1, #(DamageMeters_tables[DMT_ACTIVE]), nil);
			DamageMeters_SendReportMsg("\n");
		end

		-- Leader Report:
		if (doLeaderReport) then
			DamageMeters_DoReport(DamageMeters_ReportQuantity_Leaders, "BUFFER", false, 1, #(DamageMeters_tables[DMT_ACTIVE]), nil);
			DamageMeters_SendReportMsg("\n");
		end

		-- Event Report:
		if (doEventReport) then
			DamageMeters_DoReport(DamageMeters_ReportQuantity_Events, "BUFFER", false, 1, #(DamageMeters_tables[DMT_ACTIVE]), nil);
			DamageMeters_SendReportMsg("\n");
		end
	...
	if (createWinrarArchive) then
		...
		for index, filename in pairs(filenameList) do
		...
		for msgType, msgTable in pairs(DM_eventCaseTableDebug) do
			...
			for event, eventTable in pairs(msgTable) do
				...
				if (eventTable.unparsed ~= nil) then
					...
					for index, message in pairs(eventTable.unparsed) do
