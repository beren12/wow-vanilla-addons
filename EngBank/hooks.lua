	...
	if (reg == 1) then
		for i = 1, table.getn(EngBank_hookfunctions) do
			...
			if (func) then
				...
				EngBank_PrintDEBUG("Hook function for '"..EngBank_hookfunctions[i].." installed.");
			else
				EngBank_PrintDEBUG("** Hook function for '"..EngBank_hookfunctions[i].." SKIPPED **");
			end
		end
	elseif (reg == 0) then
		-- unregister hooks
		for i = 1, table.getn(EngBank_hookfunctions) do
			...
			if ( (func) and (EngBank_savedhookfunctions[EngBank_hookfunctions[i]]) ) then
				...
				EngBank_PrintDEBUG("Hook function for '"..EngBank_hookfunctions[i].." removed.");
			end
		end
	elseif (reg == 2) then
		-- check if hooks are registered
		EngBank_Print( "EngBank hooks:" ,1,1,0.2 );
		for i = 1, table.getn(EngBank_hookfunctions) do
			func = getglobal( "EngBank_hook_"..EngBank_hookfunctions[i] );
			func2 = getglobal( EngBank_hookfunctions[i] );

			if ( func == func2 ) then
				EngBank_Print( "  "..EngBank_hookfunctions[i].." is hooked properly." ,0,1,0.25 );
			else
				EngBank_Print( "  "..EngBank_hookfunctions[i].." is NOT hooked." ,1,0.2,0.2 );
			end
		end
	end
...
function EngBank_hook_BagSlotButton_OnClick()
	EngBank_PrintDEBUG("event: BagSlotButton_OnClick()");
...
function EngBank_hook_BagSlotButton_OnDrag()
	...
	EngBank_PrintDEBUG("event: BagSlotButton_OnDrag()");
	EngBank_UpdateWindow();
end

function EngBank_hook_CloseAllWindows()
	EngBank_PrintDEBUG("event: CloseAllWindows()");
...
function EngBank_hook_ToggleDropDownMenu(level, value, dropDownFrame, anchorName, xOffset, yOffset)
	EngBank_PrintDEBUG("event: ToggleDropDownMenu()");
	...
	if ( (adjustY ~= 0) or (adjustX ~= 0) ) then
		EngBank_PrintDEBUG("ToggleDropDownMenu() - adjusting window position by "..adjustX..", "..adjustY);
