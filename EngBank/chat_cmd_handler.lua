...
function EngBank_cmd(msg)
	...
	local cmd, params = EngBank_SplitSpace(msg);
	local param2;
	...
	elseif (cmd == "debug") then
		if (EngBank_DEBUGMESSAGES == 0) then
			...
			EngBank_Print("EngBank: Debugging messages on.");
		else
			EngBank_DEBUGMESSAGES = 0;
			EngBank_Print("EngBank: Debugging messages off.");
		end
	...
	elseif (cmd == "scale") then
		EngBankConfig["frameWindowScale"] = tonumber(params);
		EngBank_SetDefaultValues(0);
		EngBank_window_update_required = EngBank_MANDATORY;
		EngBank_UpdateWindow();
		EngBank_Print("Window scale set to: "..EngBankConfig["frameWindowScale"]);
	elseif (cmd == "set") then
		if (params == "") then
			EngBank_Print("Current settings:", 1.0,0.5,0.5);
			for key,value in EngBankConfig do
				if (not string.find(key, "__version")) then
					EngBank_Print(" "..key.." = "..value);
				end
			end
		else
			...
			if ( (EngBankConfig[params] ~= nil) and (param2 ~= nil) ) then
				...
				EngBank_Print(params.." set to '"..param2.."'", 0,1,0.5);
				EngBank_window_update_required = EngBank_MANDATORY;
				EngBank_UpdateWindow();
			else
				EngBank_Print("Invalid variable or value.  (Variables are case sensitive)", 1,0.2,0.2);
			end
		end
	elseif (cmd == "resetdefaults") then
		EngBankConfig = {};
		EngBank_SetDefaultValues(1);
		EngBank_Print("Default values loaded..");
		EngBank_Print("If you are resetting defaults because the mod wasn't running, you might have to reload your UI to make it work again.  Try /console reloadui", 1,0.2,0.2);
	...
	elseif (cmd == "test") then
		...
		for i = 1, GetNumSkillLines() do
			...
			if (isHeader) then
				EngBank_Print(skillName, 1,0,0);
			else
				EngBank_Print(skillName, 0,1,1);
			end
		end
		--]]
		...
		for key,value in EngBank_HELP[ EILocal["help_text"] ] do
			EngBank_Print(string.gsub(value, "\r", ""));
		end
	end
