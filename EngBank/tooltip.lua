...
function EngBank_ReagentHelper_ModifyItemTooltip(bag, slot, tooltipName, itm)
...
function EngBank_LootLink_SetTooltipMoney(frame, count, money, stack)
...
function EngBank_LootLink_ModifyItemTooltip(bag, slot, tooltipName, itm)
	...
	if ( ( InRepairMode() ) or
		( EngBank_ModifyTooltipsAtMerchant == 0 ) and ( MerchantFrame:IsVisible() ) ) then
		shouldModify = false;
	end
	if ( 
		...
		if ( ( not LootLinkState ) or ( not LootLinkState.HideInfo ) ) and ( name ) and ( data ) then
			if ( itm ) and ( itm["itemname"] ) then
				if ( EngBank_ShowPrice == 1 ) then
					...
					if ( money ) then
						EngBank_LootLink_SetTooltipMoney(tooltip, itm["itemcount"], money, stack);
					end
				end
...
function EngBank_RegisterCurrentTooltipSellValue(tooltip, bag, slot, itm)
...
--
-- Packrat:  If you want me to stop calling you, set EngBank_DontCallPackrat to anything.
--
function EngBank_ModifyItemTooltip(bag, slot, tooltipName, itm)
	if ( not tooltipName ) then
		tooltipName = "GameTooltip";
	end

	if ( (PackRat ~= nil) and (EngBank_DontCallPackrat == nil) ) then
		PackRat._itemEntered = { bag, slot };
		PackRat:DisplayTooltipPrice()
	end

	EngBank_ReagentHelper_ModifyItemTooltip(bag, slot, tooltipName, itm);
	EngBank_LootLink_ModifyItemTooltip(bag, slot, tooltipName, itm);
	local tooltip = getglobal(tooltipName);
	if ( not tooltip ) then	
		return;
	end
